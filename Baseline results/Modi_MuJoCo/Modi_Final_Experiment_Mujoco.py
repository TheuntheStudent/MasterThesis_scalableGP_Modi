...
def run_environment(env, num_outputs, seed = 0, pop_size = 100, num_pop = 5, num_gen = 100, batch_size = 16, max_nodes=31):
    mujoco_env = MujocoGymnaxWrapper(env, config_overrides={'njmax': 500, 'naconmax': 300000})
    ...
    #Initialize strategy
    strategy = GeneticProgramming(num_gen, pop_size, fitness_function, operator_list, variable_list, num_populations=num_pop, device_type = 'gpu', num_outputs_per_tree=num_outputs, max_nodes = max_nodes)
...
if __name__ == '__main__':
    ...
    envs = ['CartpoleBalance', 'ReacherEasy', 'SwimmerSwim', 'HopperHop', 'WalkerWalk', 'CheetahRun']
    output_sizes = [1, 2, 2, 4, 6, 6]
    seeds = [0, 26, 10]
    ...
    for i, env in enumerate(envs):
        for seed in seeds:
            ...
            start = time.time()
            run_environment(env, output_sizes[i], seed) 
            end = time.time()
