...
def run_environment(env, num_outputs, seed = 0, pop_size = 100, layer_sizes = 2, num_pop = 5, num_gen = 100, batch_size = 16, max_nodes=31):
...
if __name__ == '__main__':
    ...
    envs = ['ReacherEasy', 'SwimmerSwim', 'HopperHop', 'WalkerWalk', 'CheetahRun']
    output_sizes = [1, 1, 2, 3, 3]
    num_trees = 2 
    seeds = [0, 26, 10]
    ...
    for i, env in enumerate(envs):
        for seed in seeds:
            ...
            start = time.time()
            run_environment(env, output_sizes[i], seed, layer_sizes = num_trees) 
            end = time.time()
