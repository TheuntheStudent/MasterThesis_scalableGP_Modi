...
def run(config: Dict, wandb_run: Run) -> None:
    ...
    for _generation in range(config["n_generations"]):
        ...
        top_k = 5
        best_idx = jnp.argsort(fitness_values)[-top_k:][::-1]

        print(f"\nGeneration {_generation}:")
        for i, idx in enumerate(best_idx):
            print(f"Rank {i+1} | Fitness: {fitness_values[idx]} | Expression: {cgp_expression_from_genome(genomes[idx], config)}")
...
if __name__ == '__main__':
    ...
    entity, project = "theun-van-den-broek-radboud-university", "cgpax"
