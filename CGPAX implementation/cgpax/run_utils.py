...
def config_to_run_name(config: Dict, date: str = None):
    ...
    seed = config["seed"]
    run_name = f"{env_name}_{solver}_{ea}_{fitness}_{seed}_4.0"
    return run_name, env_name, solver, ea, fitness, seed


# methods for update notifications

async def send_telegram_text(bot: Bot, chat: str, text: str):
    await bot.send_message(chat, text=text)


def notify_update(text: str, bot: Bot = None, chat: str = None):
    print(text)
    if bot and chat:
