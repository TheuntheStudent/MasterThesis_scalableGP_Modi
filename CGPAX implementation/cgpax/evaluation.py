...
def _evaluate_program(program: Callable, program_state_size: int, rnd_key: random.PRNGKey, env: EpisodeWrapper,
    ...
    def _rollout_loop(carry: Tuple[State, jnp.ndarray, float, Tuple[int, int, int], int],
        ...
        env_state, program_state, cum_reward, rew_carry, active_episode, t = carry
        ...
        new_carry = new_state, new_program_state, cum_reward + corrected_reward, new_rew_carry, new_active_episode, t + 1
        return new_carry, corrected_reward

    (final_env_state, _, cum_reward, reward_carry, _, _), _ = lax.scan(
        f=_rollout_loop,
        init=(initial_env_state, jnp.zeros(program_state_size), initial_env_state.reward, initial_rewards_carry, 1, 0),
        xs=(),
