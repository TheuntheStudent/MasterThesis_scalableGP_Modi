...
class GeneticProgramming:
    ...
    def __init__(self, 
        ...
        self.num_outputs = num_outputs_per_tree
    ...
    def evaluate_row_from_tree(self, i: int, carry: Tuple[Array, Array, Array], node_function_list: list) -> Tuple[Array, Array]:
        ...
        # To give on the right child instead of own value, uncomment the code below 
        # and put "tree = tree.at[i, 4].set(value)" into comments

        # is_output_node = tree[i, 3] > 0
        # is_binary = tree[i, 2] >= 0
        # stored_value = jax.lax.select(
        #     is_output_node & is_binary,
        #     y,      # Store right child
        #     value   # Store value
        # )
        # tree = tree.at[i, 4].set(stored_value)
    ...
    def iterate_through_tree(self, tree: Array, data: Array) -> Array:
        ...
        x, _, result = jax.lax.fori_loop(0, self.max_nodes, self.jit_evaluate_row_from_tree, (tree, data, jnp.zeros(self.num_outputs+1))) #Iterate over the tree to compute the value of each node
    ...
    def tree_to_string_modi(self, candidate: Array) -> str:
        """
        Maps tree to string.
        ...
        def tree_to_string(tree):
            ...
            if tree[-1, 0] == 1:  # constant
                ...
                expr = self.node_to_string[tree[-1, 0].astype(int).item()]
            elif tree[-1, 2] < 0:  # Operator with one operand
                substring = tree_to_string(tree[:tree[-1, 1].astype(int) + 1])
                operator_string = self.node_to_string[tree[-1, 0].astype(int).item()]

                if operator_string[0].isalpha() or operator_string[0].isdigit():
                    expr = f"{operator_string}({substring})"
                else:
                    expr = f"({substring}){operator_string}"
                
            else:  # Operator with two operands
                substring1 = tree_to_string(tree[:tree[-1, 1].astype(int) + 1])
                substring2 = tree_to_string(tree[:tree[-1, 2].astype(int) + 1])
                operator_string = self.node_to_string[tree[-1, 0].astype(int).item()]

                if operator_string in ["+", "-", "*", "/", "**"]:
                    expr = f"({substring1}){self.node_to_string[tree[-1, 0].astype(int).item()]}({substring2})"
                else:
                    expr = f"{operator_string}({substring1}, {substring2})"
                
            output_idx = int(tree[-1, 3]) - 1
            if output_idx >= 0:
                if outputs[output_idx] == "0":
                    outputs[output_idx] = expr
                else:
                    outputs[output_idx] += " + " + expr

            return expr

        # To accurately represent equations when propagating the right child value
        # The tree_to_string(tree) function has to be commented out and replaced
        # with the tree_to_string_right(tree) function, also when calling the
        # function at the bottom of this function

        # def tree_to_string_right(tree):
        ...
        #         expr = self.node_to_string[tree[-1, 0].astype(int).item()]
        #     elif tree[-1, 2] < 0:  # Operator with one operand
        #         substring = tree_to_string_right(tree[:tree[-1, 1].astype(int) + 1])
        #         operator_string = self.node_to_string[tree[-1, 0].astype(int).item()]
        ...
        #     else:  # Operator with two operands
        #         substring1 = tree_to_string_right(tree[:tree[-1, 1].astype(int) + 1])
        #         substring2 = tree_to_string_right(tree[:tree[-1, 2].astype(int) + 1])
        #         operator_string = self.node_to_string[tree[-1, 0].astype(int).item()]

        #         if operator_string in ["+", "-", "*", "/", "**"]:
        #             full_expr = f"({substring1}){self.node_to_string[tree[-1, 0].astype(int).item()]}({substring2})"
        #         else:
        #             full_expr = f"{operator_string}({substring1}, {substring2})"
        ...
        #     full_expr = full_expr if tree[-1, 2] >= 0 else expr
                
        #     output_idx = int(tree[-1, 3]) - 1
        #     if output_idx >= 0:
        #         if outputs[output_idx] == "0":
        #             outputs[output_idx] = full_expr
        #         else:
        #             outputs[output_idx] += " + " + full_expr
    ...
    def expression_to_string(self, candidate: Array) -> str:
        ...
        string_outputs = self.tree_to_string_modi(candidate)

        for i, string in enumerate(string_outputs):
            simplified_expression = sympy.parsing.sympy_parser.parse_expr(string)
            ...
            string_outputs[i] = rounded_expression
