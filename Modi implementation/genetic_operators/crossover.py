...
def tree_crossover(tree1: Array, 
                   ...
                   num_outputs: int) -> Tuple[Array, Array]:
    """
    ...
    child1 = jax.lax.select(num_outputs==1, child1.at[:-1,3].set(jnp.zeros(len(node_ids)-1)), child1)
    child2 = jax.lax.select(num_outputs==1, child2.at[:-1,3].set(jnp.zeros(len(node_ids)-1)), child2)
...
def crossover(tree1: Array, 
    ...
    tuple of (Array, Array)
        Pair of new trees.
    """
    return jax.lax.cond(crossover_types, tree_crossover, full_crossover, tree1, tree2, keys, node_ids, operator_indices, num_outputs)
...
def safe_crossover(carry: Tuple[Array, Array, Array, Array, Array, float, Array, Array]) -> Tuple[Array, Array, Array, Array, Array, float, Array, Array]:
    ...
    """
    parent1, parent2, _, _, keys, reproduction_probability, node_ids, operator_indices, num_outputs = carry
    ...
    offspring1, offspring2 = jax.vmap(crossover, in_axes=[0, 0, 0, None, None, None, 0])(parent1, parent2, keys, node_ids, operator_indices, num_outputs, crossover_types)
    ...
    return parent1, parent2, child1, child2, keys, reproduction_probability, node_ids, operator_indices, num_outputs
...
def crossover_trees(parent1: Array, 
    ...
    _, _, child1, child2, _, _, _, _, _ = jax.lax.while_loop(check_different_trees, safe_crossover, (
        parent1, parent2, jnp.zeros_like(parent1), jnp.zeros_like(parent2), keys, reproduction_probability, jnp.arange(max_nodes), operator_indices, num_outputs))
